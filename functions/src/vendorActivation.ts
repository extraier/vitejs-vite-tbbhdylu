...
export const activateSeededVendor = onCall(
  ...
  async (req): Promise<ActivateSeededVendorResult> => {
    ...
    // Security boundary: a raw claim link can transfer ownership of a
    // seeded profile. Couples create a pending vendor request instead;
    // only administrators may mint, resend, or disclose claim links.
    if (!isAdmin(req)) {
