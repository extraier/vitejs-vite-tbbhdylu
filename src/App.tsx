...
const INITIAL_JOB_REQUESTS = [
  {
    id: 'job-1', coupleName: 'Chantal & Fiance', weddingDate: '2027年1月',
    serviceNeeded: '場地佈置', venues: ['Ritz Carlton'], budget: '$20,000 - $30,000',
    details: '急尋！需要做過Ritz Carlton嘅佈置，想要白綠色系小清新加少少香檳金，有特高樓底設計經驗優先。',
    status: 'open', proposalsCount: 2, postedAt: '2小時前'
  },
  {
    id: 'job-2', coupleName: 'Mandy & Kevin', weddingDate: '2026年11月',
    serviceNeeded: '婚禮攝影及錄影 (例如: 2P1V)', venues: ['伯大尼小教堂'], budget: '$8,000 - $12,000',
    details: '淨係需要半日伯大尼教堂行禮拍攝，想要捕捉自然情感，唔要太重filter。必須要有伯大尼拍攝經驗，知道點避開教堂規矩。',
    status: 'open', proposalsCount: 5, postedAt: '1日前'
  }
];
...
export default function App() {
  ...
  const displayTasks = useMemo(() => {
    ...
    if (searchQuery) {
      ...
      filtered = tasks.filter(t => t.title.toLowerCase().includes(q) || (t.venue && t.venue.toLowerCase().includes(q)));
    }
  ...
  const [newTaskForm, setNewTaskForm] = useState({
    categoryKey: 'other', customTitle: '', venue: '', dueDate: '', estimatedCost: '', taskType: 'vendor', assignee: ''
  });
  ...
  const renderRoleSimulator = () => (
    <div className="bg-slate-900 text-white text-sm py-2 px-4 flex flex-wrap justify-center items-center gap-4 z-50">
      <span className="font-bold flex items-center gap-1"><Users className="w-4 h-4 text-slate-400" /> 身分切換：</span>
      <button onClick={() => { setUserRole('owner'); setCurrentView('couple-checklist'); }} className={`px-3 py-1 rounded-full transition-colors ${userRole === 'owner' ? 'bg-rose-500 font-bold' : 'bg-slate-800 hover:bg-slate-700'}`}>👩🏻‍❤️‍👨🏻 主理新人</button>
      <button onClick={() => { setUserRole('collaborator'); setCurrentView('couple-checklist'); }} className={`px-3 py-1 rounded-full transition-colors ${userRole === 'collaborator' ? 'bg-indigo-500 font-bold' : 'bg-slate-800 hover:bg-slate-700'}`}>👯‍♀️ 兄弟姊妹</button>
      <button onClick={() => { setUserRole('vendor'); setCurrentView('vendor-dashboard'); }} className={`px-3 py-1 rounded-full transition-colors ${userRole === 'vendor' ? 'bg-emerald-500 font-bold' : 'bg-slate-800 hover:bg-slate-700'}`}>💼 商戶 (Vendor)</button>
    </div>
  );
            ...
            {displayTasks.map((task) => {
              ...
              return (
                <div key={task.id} onClick={() => selectTask(task)} className={`flex items-start p-3.5 rounded-xl transition-all cursor-pointer border group ${task.isCompleted ? 'bg-slate-50/50 border-transparent opacity-75' : activeCategory === task.category && activeVenue === task.venue ? 'bg-rose-50 border-rose-200 shadow-sm ring-1 ring-rose-200' : 'bg-white border-slate-200 hover:border-rose-200 hover:shadow-sm'}`}>
                  ...
                  {userRole === 'owner' && (
                    <button onClick={(e) => deleteTask(task.id, e)} className="ml-2 p-1.5 text-slate-300 hover:text-red-500 hover:bg-red-50 rounded-lg opacity-0 group-hover:opacity-100 transition-all"><Trash2 className="w-4 h-4" /></button>
                  )}
          ...
          {userRole === 'owner' && (
            <div className="mt-6 pt-5 border-t border-slate-100">
              ...
              <div className="flex gap-2 mb-3">
                 <button onClick={() => setNewTaskForm({...newTaskForm, taskType: 'vendor', assignee: ''})} className={`flex-1 py-1.5 text-xs font-bold rounded-lg border transition-colors ${newTaskForm.taskType === 'vendor' ? 'bg-rose-50 border-rose-200 text-rose-700' : 'bg-white border-slate-200 text-slate-500'}`}>💼 需要搵商戶</button>
                 <button onClick={() => setNewTaskForm({...newTaskForm, taskType: 'friend'})} className={`flex-1 py-1.5 text-xs font-bold rounded-lg border transition-colors ${newTaskForm.taskType === 'friend' ? 'bg-indigo-50 border-indigo-200 text-indigo-700' : 'bg-white border-slate-200 text-slate-500'}`}>👯‍♀️ 搵兄弟姊妹幫手</button>
              </div>
              <form onSubmit={handleAddTask} className="bg-slate-50 p-4 rounded-xl border border-slate-200">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-3">
                  <div className="sm:col-span-2">
                    <select className="w-full p-2.5 text-sm border border-slate-300 rounded-lg outline-none focus:border-rose-400 focus:ring-1 focus:ring-rose-400 bg-white" value={newTaskForm.categoryKey} onChange={(e) => setNewTaskForm({...newTaskForm, categoryKey: e.target.value})}>
                      ...
                      <optgroup label="團隊及統籌"><option value="lawyer">證婚律師</option><option value="photography">攝影及錄影</option><option value="mua">新娘化妝師</option><option value="mc">司儀</option></optgroup>
                      <optgroup label="服飾及造型"><option value="wedding_dress">婚紗及晚裝</option><option value="bridal_party_attire">姊妹裙及兄弟衫</option></optgroup>
                      <optgroup label="其他"><option value="transport">花車及旅遊巴</option><option value="invitation">喜帖</option><option value="other">✏️ 自訂項目 (其他)...</option></optgroup>
                    </select>
                  </div>
                     ...
                     {newTaskForm.taskType === 'vendor' ? (
                       ...
                       <div className="flex gap-2">
                         <select className="flex-grow p-2.5 text-sm border border-indigo-300 rounded-lg outline-none focus:border-indigo-500 bg-white text-indigo-900" value={newTaskForm.assignee} onChange={(e) => setNewTaskForm({...newTaskForm, assignee: e.target.value})}>
                           <option value="">選擇指派給...</option>{teamMembers.map(m => <option key={m.id} value={m.name}>{m.name}</option>)}
                         </select>
                         <button type="button" onClick={() => setShowInviteModal(true)} className="px-3 bg-indigo-100 text-indigo-700 rounded-lg hover:bg-indigo-200 flex items-center justify-center border border-indigo-200"><UserPlus className="w-4 h-4" /></button>
                       </div>
                     )}
                  </div>
        ...
        <div className="sticky top-28">
          {!activeCategory ? (
            <div className="bg-white rounded-2xl p-10 shadow-sm border border-slate-200 text-center flex flex-col items-center justify-center min-h-[400px] bg-[radial-gradient(ellipse_at_top_right,_var(--tw-gradient-stops))] from-rose-50/50 via-white to-white">
              <div className="w-20 h-20 bg-rose-100 rounded-full flex items-center justify-center mb-5 shadow-inner"><Briefcase className="w-10 h-10 text-rose-400" /></div>
              <h3 className="text-xl font-bold text-slate-700 mb-2">點擊左側未完成任務</h3>
              <p className="text-slate-500 max-w-sm text-sm leading-relaxed">系統會根據你填寫的項目及「指定場地」，利用 AI 自動為你篩選具備相關經驗的香港優質商戶。</p>
            </div>
          ) : (
              ...
              <div className="space-y-4 max-h-[750px] overflow-y-auto pr-2 custom-scrollbar">
                {filteredVendors.length > 0 ? (
                    ...
                    return (
                      <div key={vendor.id} className={`bg-white rounded-2xl p-6 shadow-sm border transition-all relative ${isPerfectMatch ? 'border-rose-300 ring-1 ring-rose-100' : 'border-slate-100 hover:border-slate-300'}`}>
                        ...
                        <div className="flex gap-2 mt-4">
                           <button className="flex-1 bg-slate-900 text-white py-2 rounded-xl text-sm font-bold hover:bg-slate-800">查看作品集</button>
                           <button className="flex-1 bg-rose-50 text-rose-700 border border-rose-200 py-2 rounded-xl text-sm font-bold hover:bg-rose-100">索取報價</button>
                        </div>
                      </div>
                    );
                  })
                  ...
                  <div className="bg-white rounded-2xl p-8 shadow-sm border border-slate-200 text-center flex flex-col items-center">
                    ...
                    <button onClick={() => setCurrentView('couple-jobboard')} className="bg-rose-100 text-rose-700 font-bold px-6 py-2.5 rounded-xl hover:bg-rose-200 transition-colors">不如去「求救板」出 Post 等 Vendor 搵你？</button>
                  </div>
                )}
              </div>
            </div>
        ...
        <form onSubmit={handleJobSubmit} className="space-y-5">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            <div>
              ...
              <select className="w-full p-3 rounded-xl border border-slate-300 focus:ring-2 focus:ring-rose-300 focus:border-rose-300 outline-none transition-all bg-white" value={newJobForm.serviceNeeded} onChange={e => setNewJobForm({...newJobForm, serviceNeeded: e.target.value})}>
            ...
            <div>
              ...
              <input type="text" required placeholder="例如: $15,000 - $20,000" className="w-full p-3 rounded-xl border border-slate-300 focus:ring-2 focus:ring-rose-300 focus:border-rose-300 outline-none transition-all" value={newJobForm.budget} onChange={e => setNewJobForm({...newJobForm, budget: e.target.value})} />
            </div>
            <div className="md:col-span-2">
              <label className="block text-sm font-bold text-slate-700 mb-1">指定場地經驗 (選填)</label>
              <input type="text" placeholder="例如: 伯大尼, Ritz Carlton (以逗號分隔)" className="w-full p-3 rounded-xl border border-slate-300 focus:ring-2 focus:ring-rose-300 focus:border-rose-300 outline-none transition-all" value={newJobForm.venueInput} onChange={e => setNewJobForm({...newJobForm, venueInput: e.target.value})} />
            </div>
            <div className="md:col-span-2">
              <label className="block text-sm font-bold text-slate-700 mb-1">詳細要求</label>
              <textarea rows="4" required placeholder="講多少少你嘅期望、風格、特別要求..." className="w-full p-3 rounded-xl border border-slate-300 focus:ring-2 focus:ring-rose-300 focus:border-rose-300 outline-none transition-all resize-none" value={newJobForm.details} onChange={e => setNewJobForm({...newJobForm, details: e.target.value})}></textarea>
            </div>
          </div>
      ...
      <div className="bg-white rounded-2xl p-8 shadow-sm border border-slate-200 mb-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
           <div className="bg-slate-50 rounded-xl p-5 border border-slate-200 shadow-sm">
               <p className="text-sm text-slate-500 mb-1 font-bold">總預算目標 (HKD)</p><p className="text-3xl font-black text-slate-800">${totalBudget.toLocaleString()}</p>
           </div>
           <div className="bg-rose-50 rounded-xl p-5 border border-rose-200 shadow-sm relative overflow-hidden">
               ...
               <p className="text-sm text-rose-700 mb-1 font-bold relative z-10">已確認開支 (已付款)</p><p className="text-3xl font-black text-rose-700 relative z-10">${totalSpent.toLocaleString()}</p>
           </div>
           <div className={`rounded-xl p-5 border shadow-sm ${remainingBudget < 0 ? 'bg-red-50 border-red-200' : 'bg-emerald-50 border-emerald-200'}`}>
               <p className={`text-sm mb-1 font-bold ${remainingBudget < 0 ? 'text-red-600' : 'text-emerald-700'}`}>剩餘可分配預算</p>
               <p className={`text-3xl font-black ${remainingBudget < 0 ? 'text-red-700' : 'text-emerald-600'}`}>${remainingBudget.toLocaleString()}</p>
           </div>
        </div>
        <div className="mb-10 bg-slate-50 p-5 rounded-xl border border-slate-100">
           ...
           <div className="w-full bg-slate-200 rounded-full h-4 overflow-hidden shadow-inner">
              <div className={`h-full transition-all duration-1000 ease-out relative ${budgetPercentage > 100 ? 'bg-red-500' : 'bg-rose-500'}`} style={{ width: `${Math.min(budgetPercentage, 100)}%` }}></div>
           </div>
              ...
              {tasks.map(task => (
                 <div key={task.id} className="grid grid-cols-1 sm:grid-cols-12 gap-2 sm:gap-4 p-4 bg-white hover:bg-slate-50 transition-colors items-center">
                     <div className="sm:col-span-6 flex items-start sm:items-center gap-3">
                         {task.isCompleted ? <CheckCircle2 className="w-5 h-5 text-green-500 flex-shrink-0" /> : <Circle className="w-5 h-5 text-amber-400 flex-shrink-0" />}
                         <div>
                     ...
                     <div className="sm:col-span-2 text-left sm:text-center mt-2 sm:mt-0 pl-8 sm:pl-0">
                         {task.isCompleted ? <span className="bg-green-100 text-green-700 text-xs font-bold px-2.5 py-1 rounded">已確認付款</span> : <span className="bg-amber-100 text-amber-700 text-xs font-bold px-2.5 py-1 rounded">籌備中(預算)</span>}
                     </div>
      ...
      <header className="bg-white shadow-sm sticky top-0 z-40 border-b border-slate-200">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex space-x-1 overflow-x-auto custom-scrollbar">
            {(userRole === 'owner' || userRole === 'collaborator') && <button onClick={() => setCurrentView('couple-checklist')} className={`px-4 py-3 text-sm font-bold border-b-[3px] whitespace-nowrap transition-colors ${currentView === 'couple-checklist' ? 'border-rose-500 text-rose-600' : 'border-transparent text-slate-500 hover:text-slate-700'}`}>📋 {userRole === 'owner' ? '我的籌備' : '團隊籌備'}</button>}
            {userRole === 'owner' && (
              <>
                <button onClick={() => setCurrentView('couple-budget')} className={`px-4 py-3 text-sm font-bold border-b-[3px] whitespace-nowrap transition-colors ${currentView === 'couple-budget' ? 'border-rose-500 text-rose-600' : 'border-transparent text-slate-500 hover:text-slate-700'}`}>💰 預算管理</button>
                <div className="w-px h-5 bg-slate-300 my-auto mx-2 hidden sm:block"></div>
                <button onClick={() => setCurrentView('couple-jobboard')} className={`px-4 py-3 text-sm font-bold border-b-[3px] whitespace-nowrap transition-colors flex items-center gap-1 ${currentView === 'couple-jobboard' ? 'border-rose-500 text-rose-600' : 'border-transparent text-slate-500 hover:text-slate-700'}`}>🆘 出Post求救 <span className="bg-rose-100 text-rose-600 text-[10px] px-1.5 py-0.5 rounded-full">搵Vendor</span></button>
              </>
            )}
            {userRole === 'vendor' && <button onClick={() => setCurrentView('vendor-dashboard')} className={`px-4 py-3 text-sm font-bold border-b-[3px] whitespace-nowrap transition-colors ${currentView === 'vendor-dashboard' ? 'border-emerald-500 text-emerald-600' : 'border-transparent text-slate-500 hover:text-slate-700'}`}>💼 商戶接單大堂</button>}
          </div>
        </div>
      </header>
